...
class InertiaSolver(object):
    ...
    def solve2norm_2(self, A, D, C=None, epsilon=1.e-5, adjustable=False):
        '''
        优化问题:
        min Norm(CW, 2)
        s.t. AW = D
        ...
        c = 1                                  # 可调参数初始化
        beta = numpy.ones(shape=(3, 1))
        
        item0 = numpy.matmul(C.T, C) if C is not None else numpy.identity(cols)
        item1 = numpy.matmul(A.T, A)
        item2 = numpy.matmul(A.T, D)
        item3 = A.T
...
if __name__ == '__main__':
    ...
    print(help(solver.solve2norm_2))
